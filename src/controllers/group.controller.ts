...
class GroupController {
  ...
  public async deleteUserToAGroup(
    ...
    try {
      await groupService.deleteUserToAGroup(
        req.params.idUser,
        req.params.idGroup
      );
      if (groupService === null) {
      ...
      return res.status(200).json({ message: "User removed to group" });
    } catch (error) {
      return res.status(500).json({ error: error });
    }
  }
  ...
  public async getGroupsByUser(req: Request, res: Response): Promise<Response> {
    try {
      const users: GroupDocument[] = await groupService.getGroupsByUser(
        req.params.userId
      );
      if (!users?.length) {
